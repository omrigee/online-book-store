...
public abstract class MicroService implements Runnable {
    ...
    private int id ;
    ConcurrentHashMap<Class<? extends Message>,Callback> CallbacksMap; // Map that stores
    ...
    public MicroService(String name) { //ORIGINAL CONSTRUCTOR
        this.name = name;
        this.id = 0;
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        MessageBusImpl.getInstance().subscribeBroadcast(type,this); // uses MessageBus instance to register the micro-service to the broadcast
        CallbacksMap.put(type,callback);
    }
    ...
    public synchronized int getId() { //FOR TESTING
        return id;
    }

    public void setId(int id) { //FOR TESTING
        this.id = id;
    }
    ...
    public final void run() {
        MessageBusImpl.getInstance().register(this); // registers the MicroService to the MessageBus
        initialize();
        BookStoreRunner.initializeCount.getAndIncrement();
        while (!terminated) { //MESSAGE LOOP
            try {
                    Message messageToExecute = MessageBusImpl.getInstance().awaitMessage(this);

                    Class<? extends Message> messageType = messageToExecute.getClass();
                        CallbacksMap.get(messageType).call(messageToExecute);
            }
            catch(InterruptedException e){ }
        } // end of message loop
        MessageBusImpl.getInstance().unregister(this); // after 'terminated' , the service needs to be unregistered.
